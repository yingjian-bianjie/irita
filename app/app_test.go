...
func TestIritaExport(t *testing.T) {
	db := dbm.NewMemDB()
	app := NewIritaApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, DefaultNodeHome, simapp.FlagPeriodValue, MakeEncodingConfig(), simapp.EmptyAppOptions{}, nil, interBlockCacheOpt())

	_ = setGenesis(app)

	// Making a new app object with the db, so that initchain hasn't been called
	app2 := NewIritaApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, DefaultNodeHome, simapp.FlagPeriodValue, MakeEncodingConfig(), simapp.EmptyAppOptions{}, nil, interBlockCacheOpt())
